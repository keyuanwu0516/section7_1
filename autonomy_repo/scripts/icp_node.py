...
class ICPNode(Node):
    ...
    def pcd_callback(self, msg: PointCloud2):
        ...
        # Task 2.6: Run ICP
        if self.use_open3d:
            self.transformation = open3d_icp(self.prev_pcd, current_pcd, self.transformation)
        else:
            source_np = np.asarray(self.prev_pcd.points)
            target_np = np.asarray(current_pcd.points)
